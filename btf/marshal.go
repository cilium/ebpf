...
func (e *encoder) Add(typ Type) (TypeID, error) {
	...
	iter := postorderTraversal(typ, skip)
	...
	for iter.Next() {
		...
		// Allocate IDs for cyclical types. The iterator will yield children
		// before their parents as long as there are no cycles. So the vast
		// majority of cases will not allocate any new IDs here.
		walkType(iter.Type, func(t *Type) {
			if !skip(*t) {
				allocateID(*t)
			}
		})

		// Deflate pending types.
		for !e.pending.Empty() {
			t := e.pending.Shift()
...
func (e *encoder) deflateType(typ Type) (err error) {
	...
	*raw = rawType{}
	...
	case *Enum:
		...
		raw.data, err = e.convertEnumValues(v.Values)
	...
	case *FuncProto:
		...
		raw.data, err = e.convertFuncParams(v.Params)
	...
	case *Datasec:
		...
		raw.data = e.convertVarSecinfos(v.Vars)
...
func (e *encoder) convertEnumValues(values []EnumValue) ([]btfEnum, error) {
...
func (e *encoder) convertFuncParams(params []FuncParam) ([]btfParam, error) {
...
func (e *encoder) convertVarSecinfos(vars []VarSecinfo) []btfVarSecinfo {

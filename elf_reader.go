...
func LoadCollectionSpecFromReader(rd io.ReaderAt) (*CollectionSpec, error) {
	return LoadPlatformCollectionSpecFromReader(rd, Linux)
}

func LoadPlatformCollectionSpecFromReader(rd io.ReaderAt, defaultPlatform Platform) (*CollectionSpec, error) {
	...
	for i, sec := range f.Sections {
		...
		case strings.HasPrefix(sec.Name, "version"):
			versionSection = sec
		case sec.Name == ".ebpf_go_platform":
			platformSection = sec
	...
	platformStr, err := loadSectionAsString(platformSection)
	if err != nil {
	...
	default:
		return nil, fmt.Errorf("unrecognised platform %q", platformStr)
	}
...
func (ec *elfCode) loadMaps() error {
	for _, sec := range ec.sections {
		...
		for i, offset := 0, uint64(0); i < nSym; i, offset = i+1, offset+size {
			...
			var mapType uint32
			switch {
			case binary.Read(lr, ec.ByteOrder, &mapType) != nil:
			...
			var err error
			spec.Type, err = MapTypeForPlatform(ec.platform, mapType)
			if err != nil {
				return fmt.Errorf("map %s: type: %w", mapName, err)
			}
...
func (ec *elfCode) loadBTFMaps() error {
	for _, sec := range ec.sections {
		...
		for _, vs := range ds.Vars {
			...
			mapSpec, err := ec.mapSpecFromBTF(sec, &vs, mapStruct, ec.btf, name, false)
			if err != nil {
...
func (ec *elfCode) mapSpecFromBTF(es *elfSection, vs *btf.VarSecinfo, def *btf.Struct, spec *btf.Spec, name string, inner bool) (*MapSpec, error) {
	var (
		key, value         btf.Type
	...
	for i, member := range def.Members {
		...
		case "type":
			...
			mapType, err = MapTypeForPlatform(ec.platform, mt)
			if err != nil {
		...
		case "values":
			...
			case *btf.Struct:
				...
				innerMapSpec, err = ec.mapSpecFromBTF(es, vs, t, spec, name+"_inner", true)
				if err != nil {
...
func getWindowsProgramType(sectionName string) (ProgramType, AttachType, uint32, string, error) {
	for _, t := range windowsElfSectionDefs {
		...
		return t.programType, t.attachType, 0, extra, nil
	}

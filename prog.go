...
const (
	...
	// Disable the verifier log.
	LogLevelDisabled = math.MaxUint32
)
...
type ProgramOptions struct {
	...
	//
	// Setting this to a value other than LogLevelDisabled will unconditionally
	// enable the verifier log. This will always allocate an output buffer, but
	// will result in only a single attempt at loading the program.
	LogLevel LogLevel
...
func newProgramWithOptions(spec *ProgramSpec, opts ProgramOptions, handles *handleCache) (*Program, error) {
	...
	var logBuf []byte
	if opts.LogLevel != 0 && opts.LogLevel != LogLevelDisabled {
	...
	// If the caller did not specify a log level, re-run with a branch-level
	// verifier logs and the default log buffer size.
	if opts.LogLevel == 0 {
		logBuf = make([]byte, opts.LogSize)

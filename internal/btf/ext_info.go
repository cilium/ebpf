...
func (infos FuncInfos) FuncForOffset(offset uint32) *FuncInfo {
	...
	case 0:
		return nil
	case 1:
		// In case the section only has a single function,
		// return early if the given offset is larger than the function.
		if offset >= infos[0].InsnOff {
			return &infos[0]
		}
		return nil
	}
...
func parseFuncInfoRecords(r io.Reader, bo binary.ByteOrder, recordSize uint32, recordNum uint32) (FuncInfos, error) {
	...
	for i := uint32(0); i < recordNum; i++ {
		...
		// Only the first 8 bytes of a funcInfo record are recognized,
		// discard the remainder of the record.
		if recordSize > uint32(binary.Size(fi)) {
			_, _ = io.CopyN(io.Discard, r, int64(recordSize-uint32(binary.Size(fi))))
		}
...
func parseLineInfoRecords(r io.Reader, bo binary.ByteOrder, recordSize uint32, recordNum uint32) (LineInfos, error) {
	...
	for i := uint32(0); i < recordNum; i++ {
		...
		// Only the first 8 bytes of a funcInfo record are recognized,
		// discard the remainder of the record.
		if recordSize > uint32(binary.Size(li)) {
			_, _ = io.CopyN(io.Discard, r, int64(recordSize-uint32(binary.Size(li))))
		}

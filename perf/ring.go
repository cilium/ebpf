...
type reverseReader struct {
	...
	head uint64
	// read is the position we use to read data, we start reading from head until
	// tail and we increase it each time we read.
	read uint64
...
func (rr *reverseReader) loadHead() {
	...
	// (H means header)
	//
	// The buffer is read left to right beginning from head.
	// We read until:
	// 1. Either previous_head, if less data than the buffer size were written.
	// 2. Or tail, if more data than the buffer size were written (in this case,
	// some data were overwritten because the buffer did a loop).

	if rr.head - rr.previousHead <= 0-uint64(cap(rr.ring)) {

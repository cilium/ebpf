...
func (te *TraceEvent) Close() error {
	...
	case ebpf.TracePoint:
		// Tracepoint trace events don't hold any resources.
		return nil
	}
...
func (te *TraceEvent) Attach(program *ebpf.Program) (Link, error) {
	...
	// Ignore errors, the kernel will reject the ioctl when the fd is invalid.
	kfd, _ := ifd.Value()
...
// perfEventOpenPMU opens a perf event based on a Performance Monitoring Unit.
...
// perfEventOpenTraceFS opens a perf event based on a system-wide trace event.

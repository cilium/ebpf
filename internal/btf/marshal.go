...
func (b *builder) Add(typ Type) (TypeID, error) {
	...
	// Allocate stable IDs for all types which we need to add, so that b.id
	// can find an ID even if we've not marshaled the type itself.
	for _, typ := range types {
		b.allocatedIDs[typ] = b.nextID
		b.nextID++
	}

	// Encode types in ID order.
	for _, typ := range types {
		if err := b.deflateType(typ); err != nil {
			return 0, fmt.Errorf("marshal %s: %w", typ, err)
		}
	}

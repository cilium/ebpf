...
// If filteredConfigs is nil, no filtering will occur.
// If the kconfig file is not valid, error will be returned.
func Parse(source io.ReaderAt, filteredConfigs map[string]bool) (map[string]string, error) {
	var r io.Reader
	...
	ret := make(map[string]string, len(filteredConfigs))
	...
	for s.Scan() {
		...
		if filteredConfigs != nil && len(ret) == len(filteredConfigs) {
			break
		}
	}
...
func processKconfigLine(line []byte, m map[string]string, filteredConfigs map[string]bool) error {
	...
	if filteredConfigs != nil {
		...
		_, ok := filteredConfigs[string(key)]
		if !ok {
			return nil
		}
	}

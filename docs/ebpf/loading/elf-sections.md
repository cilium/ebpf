...
eBPF binaries are no different. As an example, the section name of an individual
eBPF program determines its program type, affecting the way the program is
verified by the kernel and defining what the program is allowed to do.

{{ proj }} follows the section naming conventions established by libbpf.
...
To support encoding extra information into section names, a prefix system using
forward slashes `/` is used. For example, a Kprobe-type program meant to be
attached to the `slub_flush` kernel symbol would be assigned to an ELF section

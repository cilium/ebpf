...
func newMemory(fd, s int, ro bool) (*Memory, error) {
	...
	// Map the bpf map memory over the Go heap.
...
func allocate(size int) (unsafe.Pointer, int, error) {
	...
	if size == 0 {
		return nil, 0, errors.New("size must be greater than 0")
	}

	// Request at least a full page from the runtime and ensure the size is a
	// multiple of the page size. Otherwise, the allocated span is likely not
	// page-aligned, which we need for MAP_FIXED.
	if size != internal.Align(size, os.Getpagesize()) {
		return nil, 0, errors.New("size must be a multiple of the system's page size")
	}
...
// mapmap memory-maps the given file descriptor onto the given address and sets
// a finalizer on the address to unmap it when the backing array is no longer
// reachable.

...
func (m *Map) guessingRandomStartKey() (startKey []byte, err error) {
	var bytes1, bytes2, bytes3 []byte
	startKey = m.randomizeKey()
	bytes1, err = m.NextKeyBytes(startKey)
	if err != nil {
		return nil, err
	}
	startKey = m.randomizeKey()
	bytes2, err = m.NextKeyBytes(startKey)
	if err != nil {
		return nil, err
	}
	if !bytes.Equal(bytes1, bytes2) {
		startKey = m.randomizeKey()
		bytes3, err = m.NextKeyBytes(startKey)
		if err != nil {
			return nil, err
		}

...
// Atomic returns the type of atomic operation.
func (op OpCode) Atomic() Atomic {
	if op.Class() != StXClass || op.Mode() != AtomicMode {
		return InvalidAtomic
	}
	return Atomic(op & atomicMask)
}
...
func (op OpCode) SetAtomic(atomic Atomic) OpCode {
...
func (op OpCode) String() string {
	...
	case class.isLoadOrStore():
		...
		if atomic := op.Atomic(); atomic != InvalidAtomic {

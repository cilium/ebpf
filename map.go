...
func NewMapWithOptions(spec *MapSpec, opts MapOptions) (*Map, error) {
	btfs := make(lazyBTFLoader)
	defer btfs.close()

	return newMapWithOptions(spec, opts, btfs.load)
}

func newMapWithOptions(spec *MapSpec, opts MapOptions, newHandle func(*btf.Spec) (*btf.Handle, error)) (*Map, error) {
	switch spec.Pinning {
	case PinByName:
	...
	if spec.Type == ArrayOfMaps || spec.Type == HashOfMaps {
		...
		template, err := createMap(spec.InnerMap, nil, opts, newHandle)
		if err != nil {
			return nil, err
		}
	...
	m, err := createMap(spec, innerFd, opts, newHandle)
	if err != nil {
		return nil, err
	}
...
func createMap(spec *MapSpec, inner *internal.FD, opts MapOptions, newHandle func(*btf.Spec) (*btf.Handle, error)) (_ *Map, err error) {

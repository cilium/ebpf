...
func (l *RawLink) Info() (*Info, error) {
	...
	binary.Write(buff, internal.NativeEndian, uint64(uintptr(unsafe.Pointer(&name[0]))))
	binary.Write(buff, internal.NativeEndian, maxNameLen)
	...
	case RawTracepointType:
		...
		extra = &RawTracepointLinkInfo{Name: string(name[:nameLen])}
	case TracingType:
	...
	case IterType:
		nameLen := internal.NativeEndian.Uint32(info.Extra[8:])
		extra = &IterLinkInfo{Name: string(name[:nameLen])}
	case NetNsType:
		extra = &NetNsLinkInfo{}
	case XDPType:
		extra = &XDPLinkInfo{}

...
func (m *Map) Memory() (*Memory, error) {
	...
	i, err := m.Info()
	if err != nil {
		return nil, fmt.Errorf("getting map info: %w", err)
	}

	// Normally, maps created with BPF_F_RDONLY_PROG remain writable from user
	// space until frozen. As a security precaution, the kernel doesn't allow
	// mapping bpf map memory as read-write into user space if the bpf map has the
	// RDONLY_PROG flag.
	//
	...
	ro := (m.flags&unix.BPF_F_RDONLY_PROG > 0 || i.Frozen())
	mm, err := newMemory(m.FD(), size, ro)
	if err != nil {

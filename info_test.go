...
func BenchmarkStats(b *testing.B) {
	...
	pi, err := prog.Info()
	if err != nil {
		b.Errorf("failed to get ProgramInfo: %v", err)
	}

	lc, ok := pi.RunCount()
	if !ok {
		b.Fatalf("expected run count info to be available")
	}
	if lc != 0 {
		b.Errorf("expected a run count of 0 but got %d", lc)
	}

	lt, ok := pi.Runtime()
	if !ok {
		b.Fatalf("expected runtime info to be available")
	}
	if lt != 0 {
		b.Errorf("expected a runtime of 0ns but got %v", lt)
	}
	...
	for n := 0; n < b.N; n++ {
		...
		if err != nil {
			b.Fatalf("failed to enable stats: %v", err)
		}
		...
		if ret, _, err := prog.Test(in); ret != 42 || err != nil {
			b.Fatalf("iter %d: failed to trigger program: %v", n, err)
		}

		pi2, err := prog.Info()
		if err != nil {
			b.Fatalf("iter %d: failed to get ProgramInfo: %v", n, err)
		}

		rc, ok := pi2.RunCount()
		if !ok {
			b.Fatalf("iter %d: expected run count info to be available", n)
		}
		if rc <= lc {
			b.Fatalf("iter %d: expected run count to increase", n)
		}
		...
		rt, ok := pi2.Runtime()
		if !ok {
			b.Fatalf("iter %d: expecting runtime info to be available", n)
		}
		if rt <= lt {
			b.Fatalf("iter %d: expected runtime to increase", n)
		}
		...
		if err := stats.Close(); err != nil {
			b.Fatalf("iter %d: error closing stats fd: %v", n, err)
		}
		...
		if rc != lc {
			b.Fatalf("iter %d: did not expect run count to increase", n)
		}
		...
		if rt != lt {
			b.Fatalf("iter %d: did not expect runtime to increase", n)
		}
	}
}

...
func TestUprobeExtWithOpts(t *testing.T) {
	...
	up, err := bashEx.Uprobe("open", prog, &UprobeOptions{Offset: UprobeOffset(0x12345)})
	if err != nil {
		t.Fatal(err)
	}
	defer up.Close()
...
func TestUprobeProgramCallSharedlib(t *testing.T) {
	...
	cmd := fmt.Sprintf("/usr/bin/gdb /bin/bash --batch -ex 'printf \"%%p\", %s'", sym)
	...
	// and attach it to the ebpf program created above.
	u, err := ex.Uprobe(sym, p, &UprobeOptions{Offset: UprobeOffset(off)})
	if err != nil {
		t.Fatal(err)
	}

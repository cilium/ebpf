...
// TC Ingress hook, to monitoring TCP/UDP/ICMP network sessions and count packets.
SEC("tc")
...
// TC Egress hook, same as Ingress but with IPs and Ports inverted in the session
// identifier, to match the same entry of the Ingress in the bpf map.
SEC("tc")
int egress_prog_func(struct __sk_buff *skb) {
	...
	// Swap addresses and L4 port before doing the map lookup, to point
	// to the same entry of the Ingress hook.
	__u32 tmp  = key.saddr;

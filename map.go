...
func (spec *MapSpec) createMap(inner *sys.FD, opts MapOptions) (_ *Map, err error) {
	...
	// Some map types don't support BTF k/v in earlier kernel versions. Remove
	// BTF metadata and retry map creation.
	if errors.Is(err, sys.ENOTSUPP) && attr.BtfFd != 0 {
	...
	if err != nil {
		...
		if attr.BtfFd == 0 {
			return nil, fmt.Errorf("map create without BTF k/v: %w", err)
		}
		return nil, fmt.Errorf("map create with BTF k/v: %w", err)
	}

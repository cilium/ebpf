...
if [[ "${1:-}" = "--in-vm" ]]; then
  ...
  dmesg -C
  for i in kmalloc kmalloc_node kfree kmem_cache_alloc kmem_cache_alloc_node kmem_cache_free; do
...
if (( $# > 0 )); then
  ...
  printf -v cmd " %q" go test -v -failfast -coverpkg=./... -coverprofile="/run/output/coverage.txt" -count 1 ./...
fi

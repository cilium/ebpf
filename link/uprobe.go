...
// UprobeOffset is a manually provided offset.
func UprobeOffset(offset uint64) *uint64 {
	return &offset
}
...
//
//  ex.Uprobe("main", prog, &UprobeOptions{Offset: UprobeOffset(0x123)})
//
...
// an offset must be provided via options:
//
//  ex.Uretprobe("main", prog, &UprobeOptions{Offset: UprobeOffset(0x123)})
//
// The resulting Link must be Closed during program shutdown to avoid leaking
// system resources. Functions provided by shared libraries can currently not
// be traced and will result in an ErrNotSupported.

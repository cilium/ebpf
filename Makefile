...
# Usage: make VERSION=X.Y generate-btf
.PHONY: generate-btf
generate-btf:
	$(eval TMP := $(shell mktemp -d))
	curl -L "$(CI_KERNEL_URL)/linux-$(VERSION).bz" -o "$(TMP)/bzImage"
	...
	curl -L "$(CI_KERNEL_URL)/linux-$(VERSION)-selftests-bpf.tgz" -o "$(TMP)/selftests.tgz"

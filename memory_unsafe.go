...
func checkUnsafeMemory[T comparable](mm *Memory, off uint64) error {
	...
	align := internal.Align(off, uint64(t.Align()))
	if off != align {
		return fmt.Errorf("unaligned access of memory-mapped region: %d-byte aligned read at offset %d aligns to %d", t.Align(), off, align)
	}
...
// mapping. Only fixed-size types are supported: bools, floats, (u)int[8-64],
// arrays, and structs containing them.
//
...
// is not supported.
//
// T must be a fixed-size type, struct or array. T cannot contain Go pointers.
// [ErrInvalidType] is returned if T is not a valid type.
//

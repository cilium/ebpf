...
func TestMapPinningIncompatible(t *testing.T) {
	...
	m1, err := NewMapWithOptions(&MapSpec{
		Name:       "test",
		Type:       Hash,
		KeySize:    4,
		ValueSize:  4,
	...
	tests := []struct {
		...
		{"Type", &MapSpec{Name: "test", Type: Array, KeySize: 4, ValueSize: 4, MaxEntries: 1, Pinning: PinByName},
		...
		{"KeySize", &MapSpec{Name: "test", Type: Hash, KeySize: 8, ValueSize: 4, MaxEntries: 1, Pinning: PinByName},
			func(c *qt.C, merr *IncompatibleMapError) {
				c.Assert(merr.Existing.KeySize, qt.Equals, uint32(4))
				c.Assert(merr.Incoming.KeySize, qt.Equals, uint32(8))
			}},
		{"ValueSize", &MapSpec{Name: "test", Type: Hash, KeySize: 4, ValueSize: 8, MaxEntries: 1, Pinning: PinByName},
			func(c *qt.C, merr *IncompatibleMapError) {
				c.Assert(merr.Existing.ValueSize, qt.Equals, uint32(4))
				c.Assert(merr.Incoming.ValueSize, qt.Equals, uint32(8))
			}},
		{"MaxEntries", &MapSpec{Name: "test", Type: Hash, KeySize: 4, ValueSize: 4, MaxEntries: 2, Pinning: PinByName},
			func(c *qt.C, merr *IncompatibleMapError) {
				c.Assert(merr.Existing.MaxEntries, qt.Equals, uint32(1))
				c.Assert(merr.Incoming.MaxEntries, qt.Equals, uint32(2))
			}},
		{"Flags", &MapSpec{Name: "test", Type: Hash, KeySize: 4, ValueSize: 4, MaxEntries: 1, Pinning: PinByName, Flags: unix.BPF_F_RDONLY_PROG},
			func(c *qt.C, merr *IncompatibleMapError) {
	...
	for _, tt := range tests {
		c.Run(tt.name, func(c *qt.C) {
			m3, err := NewMapWithOptions(tt.spec, MapOptions{PinPath: tmp})
			if err == nil {
				m3.Close()

...
func readRecord(rd io.Reader) (Record, int, error) {
	...
	if err != nil {
		return Record{}, 0, fmt.Errorf("can't read event header: %w", err)
	}

	if header.isBusy() {
		return Record{}, 0, fmt.Errorf("%w", errBusy)
	}

	if !header.isDiscard() {
		/* read up to 8 byte alignment */
		data := make([]byte, internal.Align(header.dataLen(), 8))

		if _, err := io.ReadFull(rd, data); err != nil {
			return Record{}, 0, fmt.Errorf("can't read sample: %w", err)
		}

		return Record{RawSample: data[:header.dataLen()]}, header.dataLen(), nil
	}

	return Record{}, header.dataLen(), fmt.Errorf("%w", errDiscard)
}
...
func (r *Reader) Read() (Record, error) {
	...
	for {
		...
		record, rLen, err := readRecord(r.ring)
		if errors.Is(err, errBusy) {
			// the next sample in the ring is not committed yet so we
			// continue without storing the reader/consumer position
		...
		if errors.Is(err, errDiscard) {
			...
			// with an offset returned by readRecord() instead of allocating
			// data and reading/copying from the ring (which normally keeps
			// track of the consumer position).
			r.ring.storeConsumer(uint64(internal.Align(rLen, 8)))
			continue
		}

		r.ring.storeConsumer(0)

...
// parseBTF parses the .BTF section.
func parseBTF(btf io.Reader, bo binary.ByteOrder) ([]rawType, stringTable, error) {
	...
	buf := internal.NewBufferedSectionReader(rd, header.stringStart(), int64(header.StringLen))
	rawStrings, err := readStringTable(buf)
	if err != nil {
		return nil, nil, fmt.Errorf("can't read type names: %w", err)
	}

	buf = internal.NewBufferedSectionReader(rd, header.typeStart(), int64(header.TypeLen))

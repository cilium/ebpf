...
func generateTypes(spec *btf.Spec) ([]byte, error) {
	...
	gf.Identifier = internal.Identifier
...
`)
	...
	for _, o := range enums {
		fmt.Println("enum", o.id)

		var t *btf.Enum
		if err := spec.FindType(o.typeName, &t); err != nil {
			return nil, err
		}
	...
	for _, s := range structs {
		...
		if err := outputPatchedStruct(gf, w, s.id, t, s.patches); err != nil {
			return nil, fmt.Errorf("output %q: %w", s.id, err)
		}
	}
	...
	for _, s := range attrs {
		...
		case retFd:
			fmt.Fprintf(w, "func %s(attr *%s) (*FD, error) { fd, err := BPF(%s, unsafe.Pointer(attr), unsafe.Sizeof(*attr)); if err != nil { return nil, err }; return NewFD(int(fd)), nil }\n\n", s.id, id, s.cmd)
		}
	}

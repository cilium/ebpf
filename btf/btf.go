...
func loadSpecFromELF(file *internal.SafeELFFile) (*Spec, error) {
	...
	return inflateSpec(rawTypes, rawStrings, file.ByteOrder, nil, nil)
}
...
func loadRawSpec(btf io.ReaderAt, bo binary.ByteOrder,
	...
	return inflateSpec(rawTypes, rawStrings, bo, baseTypes, baseStrings)
}
...
func fixupDatasec(rawTypes []rawType, rawStrings *stringTable, sectionSizes map[string]uint32, variableOffsets map[variable]uint32) error {
	for i, rawType := range rawTypes {
		...
		name, err := rawStrings.Lookup(rawType.NameOff, nil)
		if err != nil {
			return err
		}
		...
		for j, secInfo := range secinfos {
			...
			varName, err := rawStrings.Lookup(rawTypes[id].NameOff, nil)
			if err != nil {

...
func (ec *elfCode) loadBTFMaps(maps map[string]*MapSpec, mapSections map[elf.SectionIndex]*elf.Section, spec *btf.Spec) error {
	...
	for idx, sec := range mapSections {
		...
		for _, sym := range syms {
			...
			if err != nil {
				return errors.Wrap(err, "can't get BTF")
			}

			var mapType, flags, maxEntries uint32
			for _, member := range btfMap.Type().Members {
				switch member.Name {
				case "type":
					mapType, err = uintFromBTF(member.Type)
					if err != nil {
						return errors.Wrapf(err, "map %s: can't get type", sym)
					}

				case "map_flags":
					flags, err = uintFromBTF(member.Type)
					if err != nil {
						return errors.Wrap(err, "can't get BTF map flags")
					}

				case "max_entries":
					maxEntries, err = uintFromBTF(member.Type)
					if err != nil {
						return errors.Wrap(err, "can't get BTF map max entries")
					}

				case "key":
				case "value":
				default:
					return errors.Errorf("unrecognized field %s in BTF map definition", member.Name)
				}
			}

			keySize := btf.Sizeof(btfMap.Key())
			if keySize < 0 {
				return errors.Errorf("map %s: can't get size of BTF key", sym)
			}

			valueSize := btf.Sizeof(btfMap.Value())
			if valueSize < 0 {
				return errors.Errorf("map %s: can't get size of BTF value", sym)
			}

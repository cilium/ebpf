...
func (cs *CollectionSpec) Assign(to interface{}) error {
	// Assign() only supports assigning Program and-MapSpecs, it doesn't load
	// any resources into the kernel.
	valueFor := func(typ reflect.Type, name string) (interface{}, error) {
		switch typ {
	...
	return assignValues(to, valueFor)
}
...
func (cs *CollectionSpec) LoadAndAssign(to interface{}, opts *CollectionOptions) error {
	...
	if err := assignValues(to, valueFor); err != nil {
		return err
	}
...
// valueFor is called for every such field of and must return the value
// to be assigned to the field with the given typ and name.
func assignValues(to interface{}, valueFor func(typ reflect.Type, name string) (interface{}, error)) error {
	...
	for _, field := range fields {
		...
		value, err := valueFor(field.Type, tag)
		if err != nil {
			return fmt.Errorf("field %s: %w", field.Name, err)
		}

		// Use the interface value as a unique key, since they're comparable.

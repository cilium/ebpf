...
func TestMapLoadPinnedWithOptions(t *testing.T) {
	...
	t.Run("read-only", func(t *testing.T) {
		...
		if err := array.Put(uint32(0), uint32(1)); !errors.Is(err, errno.EPERM) {
	...
	t.Run("write-only", func(t *testing.T) {
		...
		var value uint32
		if err := array.Lookup(uint32(0), &value); !errors.Is(err, errno.EPERM) {
...
func TestMapGuessNonExistentKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("%s: %s", tt.mapType, tt.name), func(t *testing.T) {
			...
			var value uint32
			if err := m.Lookup(guess, &value); !errors.Is(err, errno.ENOENT) {

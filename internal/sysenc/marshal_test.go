...
func TestUnsafeBackingMemory(t *testing.T) {
	qt.Assert(t, unsafeBackingMemory(nil, dataLayout{0, 0, 0}), qt.IsNil)
	qt.Assert(t, unsafeBackingMemory(([]byte)(nil), dataLayout{0, 0, 0}), qt.IsNil)
	qt.Assert(t, unsafeBackingMemory((*[]byte)(nil), dataLayout{0, 0, 0}), qt.IsNil)

	buf := []byte{1, 2, 3}
	qt.Assert(t, unsafeBackingMemory(buf, dataLayout{1, len(buf), len(buf)}), qt.DeepEquals, buf)
	qt.Assert(t, unsafeBackingMemory(&buf, dataLayout{1, len(buf), len(buf)}), qt.DeepEquals, buf)
}

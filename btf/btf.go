...
func New(spec *Spec) (*BTF, error) {
	...
	return &BTF{internal.NewFD(uint32(fd))}, nil
}
...
func bpfLoadBTF(attr *bpfLoadBTFAttr) (uintptr, error) {
	const _BTFLoad = 18

	return internal.BPF(_BTFLoad, unsafe.Pointer(attr), unsafe.Sizeof(*attr))
}

...
func bpfMapBatch(cmd internal.BPFCmd, m *internal.FD, inBatch, outBatch, keys, values internal.Pointer, count *uint32) error {
	fd, err := m.Value()
	if err != nil {
		return err
	}
...
func bpfMapBatchDelete(m *internal.FD, keys internal.Pointer, count *uint32) error {
	if err := hasBatchAPI(); err != nil {
		return err
	}
	return bpfMapBatch(internal.BPF_MAP_DELETE_BATCH, m, nilPtr, nilPtr, keys, nilPtr, count)
}

func bpfMapBatchLookup(m *internal.FD, inBatch, outBatch, keys, values internal.Pointer, count *uint32) error {
	if err := hasBatchAPI(); err != nil {
		return err
	}
	return bpfMapBatch(internal.BPF_MAP_LOOKUP_BATCH, m, inBatch, outBatch, keys, values, count)
}

func bpfMapBatchLookupAndDelete(m *internal.FD, inBatch, outBatch, keys, values internal.Pointer, count *uint32) error {
	if err := hasBatchAPI(); err != nil {
		return err
	}
	return bpfMapBatch(internal.BPF_MAP_LOOKUP_AND_DELETE_BATCH, m, inBatch, outBatch, keys, values, count)
}

func bpfMapBatchUpdate(m *internal.FD, keys, values internal.Pointer, count *uint32) error {
	if err := hasBatchAPI(); err != nil {
		return err
	}
	return bpfMapBatch(internal.BPF_MAP_UPDATE_BATCH, m, nilPtr, nilPtr, keys, values, count)
}
...
var hasBatchAPI = internal.FeatureTest("has batch api", "5.6", func() error {
	...
	err = bpfMapBatch(internal.BPF_MAP_UPDATE_BATCH, fd, nilPtr, nilPtr, kp, vp, &maxEntries)
	if err != nil {
		return internal.ErrNotSupported
	}
	return nil

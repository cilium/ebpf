...
`)
	...
	linkInfoExtraTypes := []struct {
		...
		{
			"RawTracepointLinkInfo", "raw_tracepoint",
		},
		{
			"TracingLinkInfo", "tracing",
		},
		{
			"XDPLinkInfo", "xdp",
		},
	...
	for _, s := range linkInfoExtraTypes {
		...
		if err := outputPatchedStruct(gf, w, s.goType, t, nil); err != nil {
			return nil, fmt.Errorf("output %q: %w", s.goType, err)
		}
	}
...
// replaceUnionWithBytes replaces the union with bytes within a struct.
// you can specify the union name or the union position (anonymous).
// the position can be -1, it means the first union in struct will be replaced
// otherwise with a given position or name, it replaces exactly the specified
// union name or union position with bytes.
func replaceUnionWithBytes(name string, position int, bytesName string) patch {
	return func(s *btf.Struct) error {
		for i, m := range s.Members {
			if (m.Name != "" && m.Name != name) ||
				(m.Name == "" && position > -1 && i != position) {
				continue
			}
		...
		return fmt.Errorf("the union name: %s / position: %d not found", name, position)
	}
}

...
func TestHaveProgramType(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.progType.String(), func(t *testing.T) {
			...
			err := HaveProgramType(testCase.progType, &ProbeOptions{Cache: cache})
...
func TestHaveProgramTypeInvalid(t *testing.T) {
	cache := btf.NewCache()
	if err := HaveProgramType(ebpf.ProgramType(math.MaxUint32), &ProbeOptions{Cache: cache}); err == nil {
...
func TestHaveProgramHelper(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.helper.String(), func(t *testing.T) {
			...
			err := HaveProgramHelper(tc.helper, &ProbeOptions{Cache: cache})
			testutils.SkipIfNotSupportedOnOS(t, err)
			if err != nil {

...
//	Kprobe("printk")
//
// As of kernel 4.17, the presence of the given symbol is only checked when
// Attaching a program later. One TraceEvent can be reused to attach an
// arbitrary number of programs.
//
// Close() must be called on the resulting TraceEvent to clean up system state
// on kernels before 4.17. On those kernels, Closing a TraceEvent created by
// Kprobe will also disconnect any Attached eBPF programs.
...
// Close() must be called on the resulting TraceEvent to clean up system state
// on kernels before 4.17. On those kernels, Closing a TraceEvent created by
// Kretprobe will also disconnect any Attached eBPF programs.

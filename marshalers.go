...
// unsafe.Pointer.
func marshalPtr(data any, length int) (sys.Pointer, error) {
	...
	// 	}
	// 	if len(buf) != length {
	// 		return sys.Pointer{}, fmt.Errorf("%T.MarshalBinary doesn't marshal to %d bytes", data, len(buf))
	// 	}
...
func makeBuffer(dst any, length int) sysenc.Buffer {
...
func marshalPerCPUValue(slice any, elemLength int) (sys.Pointer, error) {
	...
	for i := 0; i < sliceLen; i++ {
		...
		elemBytes.Copy(buf[offset : offset+elemLength])
	}

...
func assignValues(to interface{}, getValue func(typ reflect.Type, name string) (interface{}, error)) error {
	...
	assigned := make(map[interface{}]structField)
	...
	for _, field := range fields {
		...
		if af, ok := assigned[value]; ok {
			return fmt.Errorf("field %s: object %q (%v) was already assigned to field %s", field.Name, tag, value, af.Name)
		}

...
int kprobe_mkdirat(void *ctx)
{
    ...
    if (data == NULL) {
        bpf_printk("(mkdirat) map1 is empty\n");
    } else {
        bpf_printk("(mkdirat) map1 contains %d at %d\n", *data, key);
    }
    return 0;
};
...
int kretprobe_mkdirat(void *ctx)
{
    ...
    u32 *data = bpf_map_lookup_elem(&map1, &key);
    if (data == NULL) {
        bpf_printk("(mkdirat ret) inserting %d at %d in map1\n", value, key);
        bpf_map_update_elem(&map1, &key, &value, BPF_ANY);
    } else {
        bpf_printk("(mkdirat ret) data already there, nothing to do\n");
    }
    return 0;
};

...
func TestPerfEventRing(t *testing.T) {
	check := func(buffer, watermark int) {
		ring, err := newPerfEventRing(0, buffer, ReaderOptions{watermark})
		if err != nil {
			t.Fatal(err)
		}
	...
	// watermark > buffer
	_, err := newPerfEventRing(0, 8192, ReaderOptions{8193})
	if err == nil {
		t.Fatal("watermark > buffer allowed")
	}

	// watermark == buffer
	_, err = newPerfEventRing(0, 8192, ReaderOptions{8192})
	if err == nil {
		t.Fatal("watermark == buffer allowed")
	}

...
func TestUprobeGoBinaryProgramCall(t *testing.T) {
	m, p := newUpdaterMapProg(t, ebpf.Kprobe)

	// Load the executable built from './testdata/testbin'.
	ex, err := OpenExecutable("/tmp/testbin")
	if err != nil {
		t.Fatal(err)
	}

	// Open Uprobe on the symbol 'main.testfunc'
	// and attach it to the ebpf program created above.
	u, err := ex.Uprobe("main.testfunc", p, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Trigger ebpf program call.
	trigger := func(t *testing.T) {
		if err := exec.Command("/tmp/testbin").Run(); err != nil {
			t.Fatal(err)
		}
	}
	trigger(t)

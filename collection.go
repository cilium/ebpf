...
type lazyBTFLoader map[*btf.Spec]*btf.Handle

func (ldr lazyBTFLoader) load(spec *btf.Spec) (*btf.Handle, error) {
	if ldr[spec] != nil {
		return ldr[spec], nil
	}
...
func (ldr lazyBTFLoader) close() {
	for _, handle := range ldr {
		handle.Close()
	}
}
...
func lazyLoadCollection(coll *CollectionSpec, opts *CollectionOptions) (
	...
	loadMap = func(mapName string) (*Map, error) {
		...
		m, err := newMapWithOptions(mapSpec, opts.Maps, btfs.load)
		if err != nil {
	...
	loadProgram = func(progName string) (*Program, error) {
		...
		prog, err := newProgramWithOptions(progSpec, opts.Programs, btfs.load)
		if err != nil {

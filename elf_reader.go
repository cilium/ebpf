...
func (ec *elfCode) relocateInstruction(ins *asm.Instruction, rel elf.Symbol) error {
	...
	// The Undefined section is used for 'virtual' symbols that don't have any
	// physical presence in the ELF. This includes symbol references in inline
	// asm, unsatisfied forward function declarations, as well as extern kfunc
	// signature declarations using __ksym and extern kconfig variables declared
	// using __kconfig.
	case undefSection:
		if bind != elf.STB_GLOBAL {
		...
		// function declaration (see testdata/fwd_decl.c). Fall through to tagging
		// the Instruction with WithReference() so the linker can fix it up later.
		...
		// rewritten to pseudo map loads from .kconfig. If the map is present, all
		// relocations must be satisfied by it.

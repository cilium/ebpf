...
func (ec *elfCode) assignSymbols(symbols []elf.Symbol) {
	for _, symbol := range symbols {
		...
		case programSection:
			...
			// Local Basic Blocks are LLVM-internal symbols used as jump
			// targets within sections and can be ignored.
...
func (ec *elfCode) loadProgramSections() (map[string]*ProgramSpec, error) {
	...
	// Populate each program's neighbour table with pointers to the other
	// programs it directly references.
	if err := findNeighbours(progs); err != nil {
		return nil, fmt.Errorf("finding program neighbours: %w", err)
	}
...
func (ec *elfCode) loadFunctions(section *elfSection) (map[string]asm.Instructions, error) {
	...
	for _, sym := range section.symbols {
		// Load the function's instructions. Pass in the absolute offset of the
		// function within the section to allow resolving relocations.

...
func TestProgramPin(t *testing.T) {
	...
	if haveObjName() == nil {
		qt.Assert(t, qt.Equals(prog.name, t.Name()))
		if prog.name != "test" {
...
func TestProgramVerifierLog(t *testing.T) {
	check := func(t *testing.T, err error) {
		...
		qt.Assert(t, qt.IsTrue(len(ve.Log) > 0))
	}
...
func TestProgramSpecTag(t *testing.T) {
	...
	if runtime.GOOS == "windows" && info.Tag == "0000000000000000" {
		t.Skip("Windows doesn't provide Info.Tag")
	}
...
func TestProgramInstructions(t *testing.T) {
	...
	if runtime.GOOS == "windows" && errors.Is(err, ErrNotSupported) {
...
func TestProgramLoadErrors(t *testing.T) {
	...
	if runtime.GOOS != "linux" {
		t.Skip("Test relies on parsing Linux verifier output")
	}
